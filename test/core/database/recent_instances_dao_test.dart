...
void main() {
  test('touchRecentInstance keeps at most five entries ordered by recency', () async {
  ...
  test('touchRecentInstance updates name and lastUsed for existing domain', () async {
    final FluxerDatabase db = FluxerDatabase.forTesting(NativeDatabase.memory());
    addTearDown(db.close);
    final dao = db.recentInstancesDao;
    ...
    await Future<void>.delayed(const Duration(milliseconds: 2));
    await dao.touchRecentInstance(domain: 'chat.example.com', name: 'My Chat');
